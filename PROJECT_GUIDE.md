...
- From level 3 onward, eligible levels have a 28% chance to become a punch-focused level. Focus levels are separated by at least two normal levels, never repeat the previous focus type, and may emphasize jabs, crosses, hooks, body punches, or one repeated combination for the level. Defense and footwork may be sprinkled into normal combinations but never receive a dedicated Endless focus level.
- Endless has no final level or target score. Clearing a level starts its generated recovery, then the next level begins. When paused, the primary Resume control shows the current points beside “Hold to end run”; holding it ends the run. There is no separate End run button and no restart action.
...
- Standard, Compact, and Landscape main-page layouts with the same workout information and controls. Landscape gives the clock and callouts independent equal columns above the shared footer, so taller callouts do not move the clock statistics, and can swap their sides. Phones use Standard upright and Landscape sideways automatically. Mobile hides the manual layout and Auto controls. A Lock rotation switch calls the browser's orientation-lock API only where supported; iPhone Safari instead explains that rotation lock is controlled in iPhone Control Center. Never imply that merely preserving a layout prevents Safari from rotating. Desktop retains its manual selection and optional Auto layout, which uses Landscape when the workout area is at least 760 by 480 CSS pixels and Standard below that threshold.
- Callout size is a preferred maximum in every layout. Long primary or secondary text shrinks together within the reserved callout area so it cannot cover the clock, timeline, or controls. Refit when the text, layout, viewport, or size preference changes.
...
- When the selected clock size would exceed the available workout space, cap the clock font size against the same layout measured with a small clock before scaling the complete workout. This preserves the rest of the interface at its baseline size while keeping the clock as large as the current viewport permits in Standard and Compact modes.
...
- Browser-local storage keeps app preferences, custom combinations, saved workouts, Studio programs, repeatable variation seeds, workout history, program progress, and panel state available on the current device.
...
- Bundled combo tempo changes use native media playback with pitch preservation enabled. If that
  path is unavailable, playback falls back to the reliable Web Audio sprite path.
...
- Voice settings show only the selector for the active voice type.
...
`icon-themes.js` owns app icon paths, automatic folder-name labels, and gallery markup. The Pages workflow generates `assets/icons/catalog.json` and each alternate manifest by scanning the folders inside `assets/icons`. See `dist/assets/icons/README.md` when adding artwork. No page or application code should be edited for a new icon.
