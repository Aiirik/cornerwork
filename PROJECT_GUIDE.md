...
- Standard, Compact, and Landscape main-page layouts with the same workout information and controls. Landscape gives the clock and callouts independent equal columns above the shared footer, so taller callouts do not move the clock statistics, and can swap their sides. Phones use Standard upright and Landscape sideways automatically, with a local Lock view switch that keeps the view active when enabled across rotation and reloads. Mobile hides the manual layout and Auto controls. Desktop retains its manual selection and optional Auto layout, which uses Landscape when the workout area is at least 760 by 480 CSS pixels and Standard below that threshold.
