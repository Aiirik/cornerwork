...
- Standard, Compact, and Landscape main-page layouts with the same workout information and controls. Landscape gives the clock and callouts equal columns above the shared footer, and can swap their sides. Auto layout uses Landscape when the workout area is at least 760 by 480 CSS pixels, then Standard below that threshold; it leaves the manually selected layout intact when turned off.
