...
- Endless has no final level or target score. Clearing a level starts its generated recovery, then the next level begins. When paused, holding the primary Resume control ends the run; there is no separate End run button and no restart action.
