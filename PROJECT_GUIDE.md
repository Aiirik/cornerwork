...
- Standard, Compact, and Landscape main-page layouts with the same workout information and controls. Landscape gives the clock and callouts independent equal columns above the shared footer, so taller callouts do not move the clock statistics, and can swap their sides. Auto layout follows orientation on touch devices (Standard upright, Landscape sideways); with a fine pointer it uses Landscape when the workout area is at least 760 by 480 CSS pixels and Standard below that threshold. Turning Auto off restores the manually selected layout.
