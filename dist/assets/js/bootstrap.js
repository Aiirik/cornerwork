(() => {
  ...
  const blockGesture = (e) => {
    if (!zoomAllowed()) e.preventDefault();
  };
  const blockMultiTouch = (e) => {
    if (!zoomAllowed() && e.touches && e.touches.length > 1) e.preventDefault();
  };
  ...
  function addAccessibilitySetting() {
    ...
    const group = groups.find(
      (el) => el.querySelector('h2')?.textContent.trim() === 'Accessibility',
    );
    if (!group) return;
    const row = document.createElement('div');
    row.className = 'enhanced-row';
    row.innerHTML =
      '<label>Allow page zoom</label><button class="enhanced-switch" id="allowPageZoom" type="button" role="switch" aria-checked="false" aria-label="Allow page zoom"><i></i></button>';
    group.appendChild(row);
    const tip = group.querySelector('.settings-tip');
    tip?.insertAdjacentHTML(
      'beforeend',
      '<span><strong>Allow page zoom:</strong> Enables pinch-to-zoom on mobile. Zoom is locked by default to prevent accidental scaling during workouts.</span>',
    );
    const toggle = row.querySelector('#allowPageZoom');
    const sync = () => {
      const on = zoomAllowed();
      toggle.classList.toggle('on', on);
      toggle.setAttribute('aria-checked', String(on));
    };
    toggle.addEventListener('click', () => {
      localStorage.setItem(KEY, String(!zoomAllowed()));
      sync();
    });
    sync();
  }
  ...
  core.src = 'assets/js/enhancements.js?v=225';
  core.defer = true;
  core.addEventListener('load', () => {
    const endless = document.createElement('script');
    endless.src = 'assets/js/endless.js?v=6';
    endless.defer = true;
